...
//            t.y = body.get(0).y;
        ...
        if (direction == 0) {
            Point t = new Point(body.get(0).x + 1,body.get(0).y);

            body.add(0, t);
            body.remove(body.size()-1);
        }
        if (direction == 1) {
            Point t = new Point(body.get(0).x ,body.get(0).y + 1);
    ...
    public boolean isTouchingApple(Apple apple) {
        if (body.get(0).x == apple.getPoint().x && body.get(0).y == apple.getPoint().y){
            grow();
            addSpeed();
            return true;
        }else {
...
//        System.out.println("render ___________________________________________");
        ...
        gc.fillRect(body.get(0).x*10,body.get(0).y*10, 10, 10);
        gc.setFill(Color.DARKBLUE);
        for (int i = 1; i < size; i++) {
            gc.fillRect(body.get(i).x*10,body.get(i).y*10, 10, 10);
        }
